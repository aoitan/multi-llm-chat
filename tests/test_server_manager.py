...
class TestMCPServerManager(unittest.TestCase):
    ...
    def test_start_single_server(self, mock_client_class):
        """単一サーバーが起動できる"""
        manager = MCPServerManager()
        config = MCPServerConfig(
            name="test-server",
    ...
    @patch("multi_llm_chat.mcp.server_manager.MCPClient")
    def test_start_multiple_servers(self, mock_client_class):
        """複数サーバーが同時に起動できる"""
        manager = MCPServerManager()
    ...
    @patch("multi_llm_chat.mcp.server_manager.MCPClient")
    def test_get_all_tools_aggregates(self, mock_client_class):
        """全サーバーのツールリストが統合される"""
        manager = MCPServerManager()
    ...
    @patch("multi_llm_chat.mcp.server_manager.MCPClient")
    def test_call_tool_routes_correctly(self, mock_client_class):
    ...
    @patch("multi_llm_chat.mcp.server_manager.MCPClient")
    def test_call_tool_unknown_tool_raises_error(self, mock_client_class):
        """未知のツール名でエラーが発生する"""
        manager = MCPServerManager()
        config = MCPServerConfig(name="test-server", server_command="uvx", server_args=["s1"])
        manager.add_server(config)
    ...
    @patch("multi_llm_chat.mcp.server_manager.MCPClient")
    def test_stop_all_cleans_up(self, mock_client_class):
        """全サーバーが停止される"""
        manager = MCPServerManager()
        config1 = MCPServerConfig(name="server1", server_command="uvx", server_args=["s1"])
        config2 = MCPServerConfig(name="server2", server_command="uvx", server_args=["s2"])
    ...
    @patch("multi_llm_chat.mcp.server_manager.MCPClient")
    def test_tool_name_conflict_handling(self, mock_client_class):
