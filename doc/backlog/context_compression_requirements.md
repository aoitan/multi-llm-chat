...
-   **FR-3: コンテキストの枝刈り（Pruning）**
    -   **スコープ**: 枝刈りは、APIにリクエストを送信する直前に、そのリクエストのための一時的なコンテキストを生成する目的でのみ行う。永続化されているマスターの会話履歴自体は変更・破棄しない。
    -   **単位**: トークン数が**最大コンテキスト長**を超える場合、設定された圧縮方式に従って、古い会話の**ターン**（ユーザー発言とAI応答のペア）を単位として文脈から削除し、上限内に収まるように調整すること。
    -   **実行タイミング**: APIへのリクエストごとに、送信先モデルの**最大コンテキスト長**に基づいて、その都度コンテキストの評価と枝刈りを実行する。これにより、セッション中にモデルを切り替えた場合でも、各モデルの上限に動的に対応する。

-   **FR-4: システムプロンプトのトークン数チェック**
    -   システムプロンプト自体のトークン数が、選択中のモデルの**最大コンテキスト長**を超える場合、UI/CLI上でエラーを発生させてユーザーに通知し、送信を中止する。

-   **FR-5: 単一ターンの超過処理**
    -   システムプロンプトと最新の会話**ターン**だけで**最大コンテキスト長**を超える場合（例: 非常に長いテキストをペーストした場合など）、その旨をUI/CLI上でユーザーに通知し、APIへの送信を中止する。
...
-   **トークン計算方法**:
    ...
    -   **その他 (Gemini, Claude等)**: MVP段階では、暫定対応として文字数からトークン数を概算する。概算が不正確であるリスクを吸収するため、計算されたトークン数に安全マージン（バッファ）を加算する（例: `概算トークン数 * 1.2`）。このバッファ比率は `TOKEN_ESTIMATION_BUFFER_FACTOR` のような環境変数で調整可能とすることを推奨する。
...
-   **APIエラー**: API呼び出し時に（圧縮処理や安全マージン適用後もなお）トークン数上限エラーが返却された場合、その旨をユーザーに通知し、「コンテキストが長すぎます。手動で編集・短縮してください」といったメッセージを表示する。

### 7. 将来の展望

-   MVPリリース後、ユーザーのフィードバックや利用状況に応じて、より高度な**方式C: ハイブリッド（要約＋スライディングウィンドウ）**の実装を検討する。
-   各モデルプロバイダの公式トークナイザを順次導入し、トークン計算の精度を向上させる。
-   （PoC後の改善項目）概算トークン数と、APIから返却される実際のトークン数（OpenAI APIなどが提供）の差を監視し、定期的にログ出力することで、概算精度の評価や係数チューニングに役立てる。
