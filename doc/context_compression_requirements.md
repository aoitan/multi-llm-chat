...
-   **トークン計算方法**:
    ...
    -   **その他 (Gemini, Claude等)**: MVP段階では、暫定対応として文字数からトークン数を概算する。概算に用いる係数（例: 日本語は文字数 / 1.5, 英語は文字数 / 4）は、`DEFAULT_TOKEN_ESTIMATION_FACTOR`のような環境変数で調整可能とすることを推奨する。概算が不正確であるリスクと、APIエラーが発生しうることはUI上でユーザーに通知する。
