...
-   **トークン計算方法**:
    ...
    -   **注意**: `tiktoken` はOpenAIのモデル専用のトークナイザであり、Geminiなど他社のモデルではトークン数が正確に算出できない場合がある。他のモデルを利用する場合は、各モデルが提供する公式のトークナイザやAPIを利用してトークン数を算出する必要がある。
    -   **MVPでの方針**: まずはOpenAIモデルを対象として`tiktoken`で実装する。Gemini等、他のモデルについては、安全のため文字数に保守的な係数をかけた値（例: 文字数 * 1.5）をトークン数の概算値として一時的に対応し、正確なトークン計算は将来の課題とする。この概算が不正確であるリスクと、それによってエラーが発生する可能性があることを、UI上などでユーザーに通知することが望ましい。
