...
| `tests/test_core_token_context.py` | - | **新規**: 85行, 6テスト |
| `tests/test_core_legacy_api.py` | - | **新規**: 351行, 22テスト |
| `tests/test_core_facade.py` | - | **新規**: 54行, 2テスト |
| **テスト総数** | **28テスト** | **30テスト（+2）** |
| パス率 | 100% | 100% |

**注記**: テスト総数は微増（+2）しつつ、機能別に分割し保守性を向上。全285テストが通過。
