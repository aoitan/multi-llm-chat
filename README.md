...
- **MCP (Model Context Protocol) 統合**: LLMがファイルシステムにアクセスして情報を取得できます。
  - **読み取り系ツールのみ公開**: 現状、ローカルファイルの読み取り、ディレクトリ一覧取得、ファイル検索などの読み取り系操作に限定されたツールのみが公開されています
  - **セキュリティ保護**: 危険なパス（システムルート、ホームディレクトリ等）へのアクセスをブロック
  - **統合方法**: 環境変数 `MULTI_LLM_CHAT_MCP_ENABLED` などでMCPを有効化すると、CLI・Web UIの両方から利用可能です
...
**セキュリティ**: システムルート(`/`)、ホームディレクトリ、システムディレクトリ(`/etc`, `/bin`等)など、危険なパスへのアクセスはデフォルトでブロックされます。
...
- [x] **Epic 009: Coreリファクタリングとシステムプロンプト機能** ✅ 完了
  ...
  - トークンカウント表示と上限チェック
  - 後方互換性レイヤーの実装
- [x] **[Epic 004: コンテキスト圧縮とトークンガードレール](doc/specs/context_compression_spec.md)** ✅ 完了
...
- [x] **Agentic Loop & MCP対応** ✅ 完了
  - [x] [ReActループ実装（Core Logic）](doc/specs/agentic_loop_spec.md)
  - [x] [MCPクライアント実装（Infrastructure）](doc/specs/mcp_client_spec.md)
- [ ] 設定ファイルの外部化
