...
仮想環境がアクティベートされていることを確認し、`webui.py`を実行します。
```bash
python webui.py
```
...
コマンドラインで対話を行いたい場合は、`cli.py`を実行します。
```bash
python cli.py
```
...
1.  **全体リファクタリング (`system_prompt_feature_requirements.md`)**
    -   まず`core.py`, `webui.py`, `cli.py`へのファイル分割リファクタリングを行います。これが他のすべての機能の基盤となります。

2.  **システムプロンプト機能 (`system_prompt_feature_requirements.md`)**
    -   リファクタリング後のコードベースに、システムプロンプト機能のコアロジックとUIを実装します。
    -   **依存**: 全体リファクタリング

3.  **コンテキスト圧縮機能 と 会話履歴機能**
    -   以下の2つの機能は、システムプロンプト機能の実装後に並行して開発可能です。
    -   **コンテキスト圧縮機能 (`context_compression_requirements.md`)**
        -   システムプロンプトを圧縮対象から除外するロジックを含むため、システムプロンプト機能に依存します。
    -   **会話履歴の保存・管理機能 (`history_feature_requirements.md`)**
        -   システムプロンプトを履歴の一部として保存・復元するため、システムプロンプト機能に依存します。
