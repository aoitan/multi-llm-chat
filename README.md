...
1.  **全体リファクタリングとシステムプロンプト機能 (`system_prompt_feature_requirements.md`)**
    -   まず`core.py`, `webui.py`, `cli.py`へのファイル分割リファクタリングを行います。
    -   次に、システムプロンプト機能のコアロジックとUIを実装します。
    -   **依存**: なし

2.  **コンテキスト圧縮機能 (`context_compression_requirements.md`)**
    -   システムプロンプト機能を前提とし、それを圧縮対象から除外するロジックを実装します。
    -   **依存**: システムプロンプト機能

3.  **会話履歴の保存・管理機能 (`history_feature_requirements.md`)**
    -   システムプロンプトを含めた会話履歴の保存・読み込みを実装します。
    -   **依存**: システムプロンプト機能
